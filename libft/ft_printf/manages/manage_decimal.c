...
static void	get_decimal(FORMAT *f, SINTMAX *decimal)
{
	va_list copy;
	...
	if (f->flags & INTMAX)
		*decimal = va_arg((f->flags & DOLLAR) ? copy : f->list, SINTMAX);
...
static void	print_decimal(FORMAT *f, SINTMAX decimal, int size)
{
	if (size > 0)
	{
