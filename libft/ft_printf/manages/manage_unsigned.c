...
static void	get_unsigned(FORMAT *f, UINTMAX *un)
{
	va_list copy;
	...
	if (f->flags & INTMAX || f->flags & LONG1)
		*un = va_arg((f->flags & DOLLAR) ? copy : f->list, UINTMAX);
...
static void	get_size(FORMAT *f, UINTMAX un, int *size)
{
	*size = (un == 0) ? 1 : 0;
	while (un != 0)
	{
...
static void	print_unsigned(FORMAT *f, UINTMAX un, int size)
{
	if (size > 0)
	{
