...
static void	get_octal(FORMAT *f, UINTMAX *oc)
{
	va_list copy;
	...
	if (f->flags & INTMAX || f->flags & LONG1)
		*oc = va_arg((f->flags & DOLLAR) ? copy : f->list, UINTMAX);
...
static void	get_size(FORMAT *f, UINTMAX oc, int *size)
{
	*size = (oc == 0) ? 1 : 0;
...
static void	print_octal(FORMAT *f, UINTMAX oc, int size)
{
	if (size > 0)
	{
