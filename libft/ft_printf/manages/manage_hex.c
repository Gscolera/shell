...
static void	get_hex(FORMAT *f, UINTMAX *hex)
{
	va_list copy;
	...
	if (f->flags & INTMAX || f->flags & LONG1)
		*hex = va_arg((f->flags & DOLLAR) ? copy : f->list, UINTMAX);
...
static void	get_size(FORMAT *f, UINTMAX hex, int *size)
{
	*size = (hex == 0) ? 1 : 0;
	while (hex != 0)
	{
...
static void	print_hex(FORMAT *f, UINTMAX hex, int size)
{
	char value;
